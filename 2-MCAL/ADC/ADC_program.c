...
static u8 u8ADC_NumberOfChainConversions = 0;
/*Global pointer to carry the number of channels in chain conversion*/
static u8* pu8ADC_ChainNeededChannels = NULL;
/*Global flag to indicate for the chain conversion is working*/
static u8 u8ADC_ChainConversionFlag = NOT_WORKING;
/*Global counter for chain conversion*/
static u8 u8ADC_ChainCounter = 0;
...
u8 u8ADC_StartSingleConversionSynch(u8 u8Copy_Channel, u16* u16Copy_Reading)
{
	u8 u8Local_ErrorState = OK;
	u32 u32local_Counter = 0;
...
{
	/*return mV */
	u16 u16Local_Value = 0;

	/*type casting u16Copy_Value to calculate precisely the mV*/
	u16Local_Value = (u16)(((u32)u16Copy_Value * ADC_Reference) / ADC_ResolutionBit);
